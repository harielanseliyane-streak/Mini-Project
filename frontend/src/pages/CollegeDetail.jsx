      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="glass rounded-2xl p-8 mb-6 relative overflow-hidden">
          ...
          <div className="relative flex flex-wrap items-start gap-6">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 border border-white/10 flex items-center justify-center text-4xl">
              {college.logo ? <img src={`/uploads/logos/${college.logo}`} className="w-full h-full object-cover rounded-2xl" alt="logo" /> : '🏛️'}
            </div>
            <div className="flex-1">
              <div className="flex flex-wrap gap-2 mb-2">
                {college.accreditation && <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">{college.accreditation}</span>}
              </div>
              <h1 className="font-heading text-3xl font-bold text-white">{college.college_name}</h1>
              <p className="text-slate-400 mt-1">📍 {college.address}, {college.city}, {college.state}</p>
              <div className="flex flex-wrap gap-4 mt-3 text-sm text-slate-400">
                {college.website && <a href={college.website} target="_blank" rel="noreferrer" className="text-indigo-400 hover:text-indigo-300">🌐 Website</a>}
                {college.email && <span>📧 {college.email}</span>}
                {college.phone && <span>📞 {college.phone}</span>}
              </div>
            </div>
          </div>
          {college.description && <p className="relative text-slate-400 mt-5 text-sm leading-relaxed">{college.description}</p>}
        ...
        <div className="flex gap-2 mb-5 flex-wrap">
          {[['courses','📚 Courses'],['placements','💼 Placements'],['events','🎪 Events'],['posts','📢 Feed'],['media','🖼️ Media']].map(([t, l]) => (
            <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${tab === t ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>{l}</button>
          ))}
        </div>
              ...
              <tbody className="divide-y divide-white/5">
                {college.courses?.map(c => (
                  <tr key={c.id} className="hover:bg-white/5">
                    ...
                    <td className="px-4 py-3 text-slate-300">{c.duration || '—'}</td>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {college.placements?.map(pl => (
              <div key={pl.id} className="glass rounded-xl p-5">
                <div className="flex justify-between mb-3"><span className="font-heading font-bold text-white text-xl">{pl.year}</span><span className="text-emerald-400 font-bold">{pl.placement_percent}% placed</span></div>
                ...
                {pl.top_recruiters && <p className="text-slate-400 text-xs">🏢 {pl.top_recruiters}</p>}
              </div>
            ))}
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {college.events?.map(ev => (
              <div key={ev.id} className="glass rounded-xl p-5">
                ...
                {ev.description && <p className="text-slate-400 text-sm mt-2">{ev.description}</p>}
